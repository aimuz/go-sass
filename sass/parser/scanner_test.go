...
var tokens = [...]elt{
	...
	{token.FORWARD, "forward", keyword},
	{token.IMPORT, "import", keyword},
	{token.FUNCTION, "function", keyword},
	{token.EACH, "each", keyword},


	{token.RPAREN, "return", keyword},
	{token.FOR, "for", keyword},
	{token.FROM, "from", keyword},
	{token.MIXIN, "mixin", keyword},
	{token.IF, "if", keyword},

	{token.ELSE, "else", keyword},
	{token.INCLUDE, "include", keyword},
	{token.EXTEND, "extend", keyword},
	{token.DEBUG, "debug", keyword},
	{token.ERROR, "error", keyword},
}
...
func testSegments(t *testing.T, segments []segment, filename string) {
	...
	S.Init(file, []byte(src), func(pos gotoken.Position, msg string) { t.Error(Error{pos, msg}) }, dontInsertSemis)
	for _, s := range segments {
		p, _, lit := S.Scan()
		pos := file.Position(p)
		checkPos(t, lit, p, gotoken.Position{
			Filename: s.filename,
			Offset:   pos.Offset,
			Line:     s.line,
			Column:   s.column,
...
func TestStdErrorHander(t *testing.T) {
	...
	var list ErrorList
	eh := func(pos gotoken.Position, msg string) { list.Add(pos, msg) }
...
func TestNumbers(t *testing.T) {
	for _, test := range []struct {
		...
		var s Scanner
		var err string
		s.Init(fset.AddFile("", fset.Base(), len(test.src)), []byte(test.src), func(_ gotoken.Position, msg string) {
			if err == "" {
				err = msg
			}
		}, 0)
